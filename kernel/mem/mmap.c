 ...
 */
static int mmap_shrink_entry(size_t idx, u64 size, MemoryMap* mmap)
{
...
static const MemRangeTyped*
mmap_get_overlapping_entry(u64 base, u64 size, MemoryMap* mmap)
{
    for (size_t i = 0; i < mmap->entries_cnt; ++i)
    {
        const MemRangeTyped* tmp = &mmap->entries[i];
        if ((base > tmp->base && (base + size < tmp->base + tmp->size ||
                                  base < tmp->base + tmp->size)) ||
            (base <= tmp->base &&
             (base + size >= tmp->base + tmp->size || base + size > tmp->base)))
        {
            return tmp;
        }
    }
    ...
    for (size_t i = 0; i < mmap->entries_cnt; ++i)
    {
        ...
        if (e->base > tmp->base)
        {
            if (e->base + e->size < tmp->base + tmp->size)
            {
                 ...
                 * so 'tmp' is split into two and the new entry showed in
                 * between.
                 * +-----------------+     +---++===++---+
                 * |     +========+  |     |   ||   ||   |
                 * | tmp |  e     |  | ->  |tmp|| e ||tmp|
        ...
        else
        {
            ...
            else if (e->base + e->size > tmp->base)
            {
                 ...
                 * +==========+            +=======++---+
                 */
                if (mmap_shrink_entry(i, e->base + e->size - tmp->base, mmap) ==
                    0)
...
void mmap_add_entry(u64 base, u64 size, u8 type, MemoryMap* mmap)
{
    ...
    MemRangeTyped e = {.base = base, .size = size, .type = type};
...
{
    const MemRangeTyped* overlap = mmap_get_overlapping_entry(base, size, mmap);
