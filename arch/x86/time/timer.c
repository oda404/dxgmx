...
#define KLOGF(lvl, fmt, ...)                                                   \
    klogln(lvl, "timer: " fmt __VA_OPT__(, ) __VA_ARGS__)

typedef struct S_TimerSource
{
...
_INIT size_t timer_find_sources()
{
    ...
    if (best)
        KLOGF(
            INFO,
            "Found best timersource \"%s\" with id %d.",
            best->name,
            best->id);
...
{
    struct timespec ts;
    return timer_ellapsed(&ts, t) ? ts.tv_sec + ts.tv_nsec / 1000000000.0 : -1;
}

double timer_ellapsed_ms(const Timer* t)
{
    struct timespec ts;
    return timer_ellapsed(&ts, t) ? ts.tv_sec * 1000 + ts.tv_nsec / 1000000.0
                                  : -1;
}
...
{
    struct timespec ts;
    return timer_ellapsed(&ts, t) ? ts.tv_sec * 1000000 + ts.tv_nsec / 1000.0
                                  : -1;
}
...
{
    struct timespec ts;
    return timer_ellapsed(&ts, t) ? ts.tv_sec * 1000000000 + ts.tv_nsec : -1;
}
