...
__asm__(".type isr_exit, @function                       \n"
        "isr_exit:                                       \n"
...
#define IRQ_ENTRY(id)                                                          \
    ...
    {                                                                          \
        __asm__ volatile(".local irq" #id "_handler  \n"                       \
                         "pushl $0                 \n"                         \
                         "pusha                    \n"                         \
                         "pushl %esp               \n"                         \
                         "cld                      \n"                         \
...
#define SLAVE_PIC_OFFSET (MASTER_PIC_OFFSET + 8)
    ...
    idt_encode_entry(
        ...
        GDT_KERNEL_CS,
        COMMON_IRQ_GATE,
        &g_idt[0 + MASTER_PIC_OFFSET]);
    idt_encode_entry(
        (ptr)irq1,
        GDT_KERNEL_CS,
        COMMON_IRQ_GATE,
        &g_idt[1 + MASTER_PIC_OFFSET]);
    idt_encode_entry(
        (ptr)irq2,
        GDT_KERNEL_CS,
        COMMON_IRQ_GATE,
        &g_idt[2 + MASTER_PIC_OFFSET]);
    idt_encode_entry(
        (ptr)irq3,
        GDT_KERNEL_CS,
        COMMON_IRQ_GATE,
        &g_idt[3 + MASTER_PIC_OFFSET]);
    idt_encode_entry(
        (ptr)irq4,
        GDT_KERNEL_CS,
        COMMON_IRQ_GATE,
        &g_idt[4 + MASTER_PIC_OFFSET]);
    idt_encode_entry(
        (ptr)irq5,
        GDT_KERNEL_CS,
        COMMON_IRQ_GATE,
        &g_idt[5 + MASTER_PIC_OFFSET]);
    idt_encode_entry(
        (ptr)irq6,
        GDT_KERNEL_CS,
        COMMON_IRQ_GATE,
        &g_idt[6 + MASTER_PIC_OFFSET]);
    idt_encode_entry(
        (ptr)irq7,
        GDT_KERNEL_CS,
        COMMON_IRQ_GATE,
        &g_idt[7 + MASTER_PIC_OFFSET]);
    idt_encode_entry(
        (ptr)irq8,
        GDT_KERNEL_CS,
        COMMON_IRQ_GATE,
        &g_idt[8 + MASTER_PIC_OFFSET]);
    idt_encode_entry(
        (ptr)irq9,
        GDT_KERNEL_CS,
        COMMON_IRQ_GATE,
        &g_idt[9 + MASTER_PIC_OFFSET]);
    idt_encode_entry(
        (ptr)irq10,
        GDT_KERNEL_CS,
        COMMON_IRQ_GATE,
        &g_idt[10 + MASTER_PIC_OFFSET]);
    idt_encode_entry(
        (ptr)irq11,
        GDT_KERNEL_CS,
        COMMON_IRQ_GATE,
        &g_idt[11 + MASTER_PIC_OFFSET]);
    idt_encode_entry(
        (ptr)irq12,
        GDT_KERNEL_CS,
        COMMON_IRQ_GATE,
        &g_idt[12 + MASTER_PIC_OFFSET]);
    idt_encode_entry(
        (ptr)irq13,
        GDT_KERNEL_CS,
        COMMON_IRQ_GATE,
        &g_idt[13 + MASTER_PIC_OFFSET]);
    idt_encode_entry(
        (ptr)irq14,
        GDT_KERNEL_CS,
        COMMON_IRQ_GATE,
        &g_idt[14 + MASTER_PIC_OFFSET]);
    idt_encode_entry(
        (ptr)irq15,
        GDT_KERNEL_CS,
        COMMON_IRQ_GATE,
