...
/* ICW1 Programs the basic operation (1 byte, bit 4 must be set for init
 * sequence) */
...
void pic8259_remap(u8 master_offset, u8 slave_offset)
{
    ...
    /* ICW1 for the master */
    port_outb(PIC_ICW1_INIT | PIC_ICW1_ICW4_NEEDED, PIC_MASTER_PORT_COMMAND);
    /* ICW1 for the slave */
    port_outb(PIC_ICW1_INIT | PIC_ICW1_ICW4_NEEDED, PIC_SLAVE_PORT_COMMAND);
